...
def validate_game_scores(
    ...
    *,
    ...
    if len(score_list) != len(participant_list):
    ...
    if set(ids) != set(participant_list):
        raise DomainValidationError("Enter exactly one score for every participant.")
    ...
    winner_score = next(score for score in score_list if score.user_id == winner_id)
    if winner_score.total_points < (rules.target_points or 0):
