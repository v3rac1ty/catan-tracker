...
A Discord bot for that tracks Catan wins and losses and ranks players by win rate. Seasons run for a set period with a minimum-games threshold (default 5) for eligibility; when a season ends, the lowest-ranked eligible player buys food for the top-ranked player. The bot also schedules game nights with RSVPs and reminders.
