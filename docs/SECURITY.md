...
- **Buttons and IDs:** Dynamic item templates accept only fixed actions and decimal IDs (`GameActionButton.from_custom_id`/`GameNudgeButton.from_custom_id` in `src/catan_bot/views/game_confirm.py`; `EventRsvpButton.from_custom_id` in `src/catan_bot/views/event_rsvp.py:15-83`). Callbacks derive the guild from the interaction and repeat service-side authorization.
...
| Low | the `@app_commands.checks.cooldown` on `GameCog.report_command` in `src/catan_bot/cogs/game_cog.py`; event commands in `src/catan_bot/cogs/event_cog.py:30-127`; read commands in `src/catan_bot/cogs/stats_cog.py:15-56`; `src/catan_bot/db/pool.py:16-20` | Only game reporting has a command cooldown. Repeated event, RSVP, and read commands can consume the small pool and Discord/API quota. | Add per-user and per-guild rate limits for expensive commands and metrics for rejected requests. Keep database timeouts and pool limits as defense in depth. |
| Low | `src/catan_bot/services/event_service.py:224-239`; the at-most-once tradeoff documented in `src/catan_bot/scheduler.py`'s module docstring, and the claim-then-deliver ordering in its `_send_reminders`/`_send_reminder` | Reminder delivery can be lost after a committed claim, and a crash before an announcement is marked can duplicate it. These are at-most-once and duplicate-delivery risks in a single-bot deployment. | Add an outbox or durable delivery state with retry/dead-letter handling and idempotency keys; alert operators when delivery is abandoned. |

## SQL injection category review

| Category | Result | Evidence |
|---|---|---|
| Comments, quote breaking, tautologies, stacked statements | PASS | `tests/integration/test_sql_injection.py:50-70`; `tests/integration/test_privileges.py:138-163`; `tests/static/test_no_dynamic_sql.py` |
...
| Out-of-band, file reads, `COPY ... PROGRAM` | PASS | `tests/integration/test_sql_injection.py:67`; `tests/integration/test_privileges.py:62-66`; role settings in `db/roles.sql:57-71,99-114` |
...
| Privilege escalation and destructive SQL | PASS | `db/roles.sql:103-132`; `tests/integration/test_privileges.py:28-79,93-185` |
